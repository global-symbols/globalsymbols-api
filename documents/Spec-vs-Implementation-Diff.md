...
| `expand` parameter on labels endpoints | `GET /v1/labels/search` and `GET /v1/labels/:id` support `expand` | `expand` accepted on Go `GET /api/v2/labels/search` and `GET /api/v2/labels/{id}`; `picto.symbolset` embeds nested symbolset via batched lookup; other paths ignored | Match (Rails-compatible path implemented) |
...
| `Picto.native_format` values | Spec allows `svg` or `png` | Runtime infers `native_format` from image filename extension via `internal/db/image_formats.go`; unknown/missing extensions fall back to `png` | Partial match (fallback may mask unknown formats) |
